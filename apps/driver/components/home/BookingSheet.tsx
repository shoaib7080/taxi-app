...
export const BookingSheet = ({
  ...
  const [bookRide, { loading: bookingLoading }] = useMutation(
    BOOK_INSTANT_RIDE_MUTATION,
    {
      onCompleted: (data) => {
        setCurrentRideId(data.bookInstantRide.id); // Save ID
        setIsSearching(true); // Start UI Animation
      },
      onError: (err) => Alert.alert("Booking Error", err.message),
    },
  );

  // 3. POLLING: CHECK STATUS (Only when rideId exists)
  const {
    data: rideData,
    stopPolling,
    error,
  } = useQuery<any>(GET_RIDE_STATUS, {
    variables: { id: currentRideId || "" },
    skip: !currentRideId, // Don't run if no ride
    pollInterval: 2000, // Check every 2 seconds
    fetchPolicy: "network-only",
  });

  useEffect(() => {
    console.log("Ride ID Changed:", currentRideId);
    if (currentRideId) {
      console.log("Starting polling for:", currentRideId);
    } else {
      stopPolling();
    }
  }, [currentRideId]);
  ...
  useEffect(() => {
    ...
    if (rideData) console.log("Ride Data Update:", JSON.stringify(rideData));
  }, [rideData, error]);

  // Handle Polling Updates via Effect
  useEffect(() => {
    // console.log("🔥 Polling Effect Triggered", {
    //   hasData: !!rideData?.getRide,
    //   currentRideId,
    //   status: rideData?.getRide?.status,
    // });

    if (!rideData?.getRide || !currentRideId) return;

    const status = rideData.getRide.status;
    console.log("Polling Status:", status); // Debug log

    // A. RIDE ACCEPTED -> NAVIGATE
    if (status === "ACCEPTED" || status === "CONFIRMED") {
      console.log("✅ Ride Accepted! Navigating...");
      stopPolling();
      setIsSearching(false);

      // Store ID locally before clearing state
      const rideIdToNav = currentRideId;
      setCurrentRideId(null);

      router.push({
        pathname: "/ride-details",
        params: { rideId: rideIdToNav },
      });
    }

    // B. RIDE CANCELLED/TIMEOUT -> RESET
    if (status === "CANCELLED") {
      console.log("❌ Ride Cancelled/Timeout");
      stopPolling();
      setIsSearching(false);
      setCurrentRideId(null);
  ...
  const handleBookPress = () => {
    // 1. If searching, user can cancel
    if (isSearching) {
      setIsSearching(false);
      setCurrentRideId(null);
      stopPolling();
    ...
    if (!userLocation || !destination) {
      Alert.alert("Missing Details", "Please select a pickup and drop-off.");
      return;
    }

    // 3. Trigger Booking
    const fare = calculateFare(tripDetails?.distance, 1); // Get real multiplier from selectedVehicle

    bookRide({
      variables: {
        input: {
          originLat: userLocation.coords?.latitude || userLocation.latitude,
          originLng: userLocation.coords?.longitude || userLocation.longitude,
  ...
  useEffect(() => {
    if (isSearching) {
      progress.value = withTiming(
        1,
        { duration: 60000, easing: Easing.linear },
        (finished) => {
          if (finished) {
